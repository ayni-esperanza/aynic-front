...
import { UpdatePurchaseOrderData } from '../types';
  ...
  useEffect(() => {
    const fetchPurchaseOrder = async () => {
      try {
        ...
        setFormData({
          numero: data.numero ?? '',
          termino_referencias: data.termino_referencias ?? '',
        });
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: UpdatePurchaseOrderData = {
        ...
        termino_referencias: formData.termino_referencias ?? '',
      };
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg border border-gray-200 p-6 space-y-6">
        <div className="grid grid-cols-1 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Término / Referencias</label>
            <textarea
              name="termino_referencias"
              value={formData.termino_referencias || ''}
              onChange={handleInputChange}
              rows={3}
              className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[#18D043] focus:border-transparent border-gray-300"
